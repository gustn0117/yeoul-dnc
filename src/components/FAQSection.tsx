...
export default function FAQSection() {
  ...
  return (
    <section className="relative py-20 sm:py-24 lg:py-32 bg-linear-to-b from-white via-[#f5f9ff] to-white overflow-hidden">
      ...
      <div className="absolute bottom-20 left-0 w-80 h-80 bg-violet-100/30 rounded-full blur-3xl" />
        ...
        <div className="space-y-3 sm:space-y-4">
          {faqs.map((f, i) => {
            ...
            return (
              ...
              >
                {/* 3D depth */}
                <div className={`absolute inset-0 translate-x-0.5 translate-y-1 rounded-2xl bg-linear-to-br ${f.color} opacity-15 blur-[2px]`} />
                ...
                >
                  <div className={`absolute -top-10 -right-10 w-32 h-32 rounded-full bg-linear-to-br ${f.color} opacity-10 blur-2xl transition-opacity duration-300 ${isOpen ? "opacity-20" : ""}`} />

                  <div className="relative p-5 sm:p-6">
                    <div className="flex items-start gap-4">
                      {/* Q badge */}
                      <div className="relative shrink-0">
                        <div className={`absolute inset-0 translate-x-0.5 translate-y-0.5 rounded-xl bg-linear-to-br ${f.color} opacity-60 blur-[1px]`} />
                        <div className={`relative w-10 h-10 sm:w-11 sm:h-11 rounded-xl bg-linear-to-br ${f.color} flex items-center justify-center shadow-md`}>
                      ...
                      <div className="flex-1 min-w-0 pt-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-[10px] font-extrabold bg-linear-to-br ${f.color} bg-clip-text text-transparent tracking-widest uppercase`}>
                            {f.tag}
                          </span>
                      ...
                      {/* Chevron */}
                      <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${isOpen ? `bg-linear-to-br ${f.color} rotate-180` : "bg-slate-50"}`}>
                    ...
                    {isOpen && (
                      <div className="animate-accordion mt-4 pt-4 border-t border-slate-100 flex gap-3 sm:gap-4">
                        <div className="shrink-0 w-10 sm:w-11 flex justify-center">
                          <div className={`w-7 h-7 rounded-lg bg-linear-to-br ${f.color} flex items-center justify-center shadow-sm opacity-80`}>
